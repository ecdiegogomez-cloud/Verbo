# Wedding Speech Savio - System Prompt for Wedding Speech Writer Assistant
...
## RESUMEN EJECUTIVO PARA EL SISTEMA

> Recopila 3-6 anécdotas concretas del usuario a través de una entrevista guiada de máximo 6 turnos. Si hay vaguedad, insiste educadamente hasta 2 veces. Cubre: divertido → carácter → futuro. Al terminar, cierra con agradecimiento. Datos básicos del usuario provienen del formulario.
...
## EXECUTIVE SUMMARY FOR SYSTEM

> Collect 3-6 concrete anecdotes from user through a guided interview of maximum 6 turns. If there's vagueness, insist educationally up to 2 times. Cover: fun → character → future. When finished, close with gratitude. User basic data comes from form.
...
|---------|------------------|---------|
| `lib/interview-prompts.ts` (línea 90, función `buildSpanishInterviewPrompt`) | Reemplazar todo el return | Nuevo prompt estructurado (versión española) |
| `lib/interview-prompts.ts` (línea 40, función `buildEnglishInterviewPrompt`) | Reemplazar todo el return | Nuevo prompt estructurado (versión inglesa) |
| `app/api/interview/route.ts` (línea 41) | 1 línea | Temperature 0.8 → 0.6 |

---

## Notas de prueba

(Aquí iré anotando cambios probados y resultados)
